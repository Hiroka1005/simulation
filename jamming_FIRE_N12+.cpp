...
void output_coord(double (*x)[dim],double *a,double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"coord_FIRE_Ntot%d+_phi%.3f_1.dat",N1+N2,phi);
...
void output_potential(double U,double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"potential_FIRE_Ntot%d+_1.dat",N1+N2);
  file.open(filename,std::ios::app);
  file << phi << "\t" << U << std::endl;
  file.close();
}
...
void output_pressure(double p,double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"pressure_FIRE_Ntot%d+_1.dat",N1+N2);
  file.open(filename,std::ios::app);
  file << phi << "\t" << p << std::endl;
  file.close();
}
...
void output_criticalpoint(double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"criticalpoint_FIRE_Ntot%d+_1.dat",N1+N2);
  file.open(filename,std::ios::app);
  file << phi << std::endl;
  file.close();
}
