...
int main(){
  ...
  for(i=0;i<10;i++){  //-6 <= log{10}(phi-phi_J) <= -5
    affine_transformation(x,&L,phi,dphi_a*5.0e-4);
    FIRE(list,x,x_update,f,a,&U,&p,&disp_max,&f_tot,L);
    dphi+=dphi_a*5.0e-4;
    phi+=dphi_a*5.0e-4;
    output_coord(x,a,dphi);
    //output_potential(U,dphi);
    output_pressure(p,dphi);
  }
  for(i=0;i<9;i++){  //-5 <= log{10}(phi-phi_J) <= -4
    affine_transformation(x,&L,phi,dphi_a*5.0e-3);
    FIRE(list,x,x_update,f,a,&U,&p,&disp_max,&f_tot,L);
    dphi+=dphi_a*5.0e-3;
    phi+=dphi_a*5.0e-3;
    output_coord(x,a,dphi);
  ...
  for(i=0;i<9;i++){  //-4 <= log{10}(phi-phi_J) <= -3
    affine_transformation(x,&L,phi,dphi_a*5.0e-2);
    FIRE(list,x,x_update,f,a,&U,&p,&disp_max,&f_tot,L);
    dphi+=dphi_a*5.0e-2;
    phi+=dphi_a*5.0e-2;
    output_coord(x,a,dphi);
  ...
  for(i=0;i<9;i++){  //-3 <= log{10}(phi-phi_J) <= -2
    affine_transformation(x,&L,phi,dphi_a*5.0e-1);
    FIRE(list,x,x_update,f,a,&U,&p,&disp_max,&f_tot,L);
    dphi+=dphi_a*5.0e-1;
    phi+=dphi_a*5.0e-1;
    output_coord(x,a,dphi);
