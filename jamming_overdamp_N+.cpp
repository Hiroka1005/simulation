...
void affine_transformation(double (*x)[dim],double *L,double phi,double dphi){
  ...
  *L*=sqrt(phi/(phi+dphi));
  for(i=0;i<N1+N2;i++){
    for(j=0;j<dim;j++){
      x[i][j] *= sqrt(phi/(phi+dphi));
    }
  }
}

void affine_geometric(double (*x)[dim],double *L){
  int i,j=0;
...
void eom_overdamp(int (*list)[N1+N2],double (*x)[dim],double (*x_update)[dim],double (*f)[dim],double *a,double *U,double *P,double *Fmax,double *disp_max,double L){
...
void output_coord(double (*x)[dim],double *a,double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"coord_od_debug.dat");
...
void output_dr(int step,double (*x)[dim],double *a,double L,double phi){
  double dx,dy,dr,aij;
  int i,j;
  int k=0;
  char filename[128];
  std::ofstream file;
  sprintf(filename,"dr_od_phi%.3f_step%d.dat",phi,step);
  file.open(filename);
  for(i=0;i<N1+N2;i++){
    for(j=0;j<i;j++){
      dx=x[i][0]-x[j][0];
...
void output_criticalpoint(double phi){
  char filename[128];
  std::ofstream file;
  sprintf(filename,"criticalpoint_od_Ntot%d+_debug.dat",N1+N2);
  file.open(filename,std::ios::app);
  file << phi << std::endl;
  file.close();
}

void initialize(int (*list)[N1+N2],double (*x)[dim],double (*f)[dim],double *a,double *U,double *P,double *Fmax,double *phi,double L){
  *phi=phi0;
  ini_array(x);
  ini_coord(x,L);
  list_verlet(list,x,L);
...
void training(int (*list)[N1+N2],int step,double (*x)[dim],double (*x_update)[dim],double (*f)[dim],double *a,double *U,double *P,double *phi,double dphi,double *Fmax,double *disp_max,double *L){
  calc_force(list,x,f,a,&(*U),&(*P),&(*Fmax),*L);
  while (*Fmax>1.0e-12){
    eom_overdamp(list,x,x_update,f,a,&(*U),&(*P),&(*Fmax),&(*disp_max),*L);
  }
  //output_coord(step,x,a,*phi);
  //output_dr(step,x,a,*L,*phi);
...
int main(){
  double x[N1+N2][dim],x_update[N1+N2][dim],f[N1+N2][dim],a[N1+N2],U,P,phi,phi_J,F,Fmax;
  double L=sqrt((N1*a1/2.0*a1/2.0+N2*a2/2.0*a2/2.0)*M_PI/phi0),disp_max=0.0;
  char filename[128];
  int list[N1+N2][N1+N2],step;

  set_diameter(a);
  initialize(list,x,f,a,&U,&P,&Fmax,&phi,L);

  while(P<1.0e-9){
    training(list,step,x,x_update,f,a,&U,&P,&phi,dphi_a,&Fmax,&disp_max,&L);
  }
